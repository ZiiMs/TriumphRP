...
-- colors
local r,g,b,a = 166,166,255,150 -- rgba color

-- ranges
local voice = {default = 5.0, shout = 25.0, whisper = 1.0, current = 0}
...
Citizen.CreateThread(function()
    while true do
        ...
        if voice.current == 0 then
            drawTxt(1.435, 1.46, 1.0,1.0,0.4, "~w~Voice: ~b~Normal", 255, 255, 255, 255)
        elseif voice.current == 1 then
            drawTxt(1.435, 1.46, 1.0,1.0,0.4,"~w~Voice: ~b~Shout", 255, 255, 255, 255)
        elseif voice.current == 2 then
            drawTxt(1.435, 1.46, 1.0,1.0,0.4,"~w~Voice: ~b~Whisper", 255, 255, 255, 255)
        end
    end
end)
        ...
        while true do
            ...
            local takeaway = 0.95
                ...
                if NetworkIsPlayerTalking(id) then
                    DrawMarker(25,x2,y2,z2 - takeaway, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 10.3, 166, 166, 255, 100, 0, 0, 2, 0, 0, 0, 0)
                end
                if NetworkIsPlayerTalking(PlayerId()) then
                    DrawMarker(25,coords.x,coords.y,coords.z - takeaway, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 10.3, 166, 166, 255, 100, 0, 0, 2, 0, 0, 0, 0)
                end
                    end  
                end
            end
